...
// 会话持久化本地接口；地址小驼峰，adapter 转到 Tauri Command。
...
export async function listChatConversations(options: {
  ...
  const result = await requestClient.post<{ conversations: PersistedChatConversation[] }>(
...
export async function upsertChatConversation(options: {
  ...
  const result = await requestClient.post<{ conversation: PersistedChatConversation }>(
...
export async function deleteChatConversation(id: string): Promise<void> {
  await requestClient.post<{ ok: boolean }>("chatDeleteConversation", { id });
}
...
export async function reorderChatConversations(options: {
  ...
  await requestClient.post<{ ok: boolean }>("chatReorderConversations", {
    input: {
      scope: options.scope,
      projectId: options.projectId ?? null,
