...
// SSH 密钥本地接口：Command 生成/扫描 + Tauri Store 登记元数据（不含私钥内容）。
...
export async function createSshKey(name: string, passphrase: string): Promise<SshKeyRecord[]> {
  ...
  const material = await requestClient.post<SshKeyMaterial>("sshKeyGenerate", {
    input: {
      name: trimmedName,
      passphrase,
    },
...
export async function scanLocalSshKeys(): Promise<SshKeyScanResult> {
  return requestClient.post<SshKeyScanResult>("sshKeyScanLocal");
}
...
export async function importSshKeyFromDisk(): Promise<SshKeyRecord[] | null> {
  ...
  const material = await requestClient.post<SshKeyMaterial>("sshKeyReadPublic", {
    input: { path: selected },
  });
...
export async function deleteSshKey(id: string): Promise<SshKeyRecord[]> {
  ...
  if (target.origin === "generated") {
    await requestClient.post<{ ok: boolean }>("sshKeyDelete", {
...
export async function changeSshKeyPassphrase(
  ...
  const result = await requestClient.post<{ hasPassphrase: boolean }>("sshKeyChangePassphrase", {
    input: {
