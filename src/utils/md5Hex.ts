/**
 * 浏览器侧 MD5（hex），供 Gravatar 兼容镜像使用。
 * Web Crypto 不提供 MD5；此处为纯函数实现，无第三方依赖。
 */
...
function wordToHex(word: number): string {
  let hex = "";
  for (let i = 0; i < 4; i += 1) {
    hex += ((word >> (i * 8 + 4)) & 0x0f).toString(16);
    hex += ((word >> (i * 8)) & 0x0f).toString(16);
  }
  return hex;
}

function cmn(q: number, a: number, b: number, x: number, s: number, t: number): number {
  const n = (((a + q + (x | 0) + t) | 0) << s) | (((a + q + (x | 0) + t) | 0) >>> (32 - s));
  return (n + b) | 0;
}
