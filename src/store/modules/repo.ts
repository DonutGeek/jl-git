...
function ensureSelectedCommitDetail(): void {
  const state = useRepoStoreWithOut();
...
export function restoreRepoSessionHistory(repoPath: string): boolean {
  const cached = repoSessionCache.get(repoPath);
  if (!cached || useRepoStoreWithOut().repoPath !== repoPath) {
    return false;
  }
...
export function beginRepoSwitch(repoPath: string, resetCurrent = false): void {
  const current = useRepoStoreWithOut();
...
type RepoSet = (
  partial: Partial<RepoStoreState> | ((state: RepoStore) => Partial<RepoStoreState> | RepoStore),
) => void;

function repoSet(partial: Parameters<RepoSet>[0]): void {
  applyStorePatch(useRepoStoreWithOut(), partial);
}

function repoGet(): RepoStore {
  return useRepoStoreWithOut();
}

export const useRepoStore = defineStore("repo", {
  state: (): RepoStoreState => ({ ...initialState }),
  actions: createRepoActions(repoSet, repoGet),
});

export function useRepoStoreWithOut() {
  return useRepoStore(store);
}

function createRepoActions(set: RepoSet, get: () => RepoStore): RepoStoreActions {
  return {
    reset() {
        ...
        if (restoredBackup) {
          const wrapped =
            error && typeof error === "object"
              ? { ...error, restoredLintStagedBackup: true }
              : error;
          throw wrapped;
        }
        throw error;
    ...
    async push(options) {
      ...
      const statusForBranch =
        get().repoPath === repoPath ? get().status : (session?.status ?? null);
