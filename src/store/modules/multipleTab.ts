...
type OpenTabsActions = Omit<OpenTabsState, keyof OpenTabsData>;

const OPEN_TABS_STORAGE_KEY = "jlgit-open-tabs";
const OPEN_TABS_PERSIST_VERSION = 2;

function tabsSet(
  partial:
    Partial<OpenTabsData> | ((state: OpenTabsState) => Partial<OpenTabsData> | OpenTabsState),
): void {
  applyStorePatch(useOpenTabsStoreWithOut(), partial);
}

function tabsGet(): OpenTabsState {
  return useOpenTabsStoreWithOut();
}
...
export const useOpenTabsStore = defineStore("openTabs", {
  ...
  actions: ((set, get): OpenTabsActions => ({
  ...
  persist: {
    ...
    pick: ["tabs", "lastActiveTabId"],
    serializer: {
      deserialize(value: string): Pick<OpenTabsData, "tabs" | "lastActiveTabId"> {
        const version = readZustandPersistVersion(value);
        const persisted = deserializeZustandPersist<unknown>(value);
        const tabs = readPersistedTabs(persisted, version);
        return {
          tabs,
          lastActiveTabId: readLastActiveTabId(persisted, tabs),
        };
      },
      serialize: (value) =>
        serializeZustandPersist(
          { tabs: value.tabs, lastActiveTabId: value.lastActiveTabId },
          OPEN_TABS_PERSIST_VERSION,
        ),
    },
  },
});

export function useOpenTabsStoreWithOut() {
  return useOpenTabsStore(store);
}
