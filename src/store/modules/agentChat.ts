...
export const useAgentChatStore = defineStore("agentChat", {
  state: (): Pick<
    AgentChatState,
    "conversationsByProjectId" | "activeConversationIdByProjectId"
  > => ({
    conversationsByProjectId: {},
    activeConversationIdByProjectId: {},
  }),
  actions: {
    hydrateProject(projectId: string, conversations: readonly AgentConversation[]) {
    ...
    createConversation(projectId: string, conversation: AgentConversation) {
      applyStorePatch(this, (state) => {
    ...
    ensureDefaultConversation(projectId: string) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
    ...
    setActiveConversation(projectId: string, conversationId: string) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
        if (
    ...
    setConversationPinned(projectId: string, conversationId: string, pinned: boolean) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
        const index = conversations.findIndex((conversation) => conversation.id === conversationId);
        if (index < 0) {
    ...
    reorderConversations(projectId: string, activeId: string, overId: string) {
      if (activeId === overId) {
        return;
      }
      applyStorePatch(this, (state) => {
    ...
    appendMessage(projectId: string, conversationId: string, message: AgentChatMessage) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
    ...
    removeMessage(projectId: string, conversationId: string, messageId: string) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
        return {
          conversationsByProjectId: {
    ...
    truncateMessagesAfter(projectId: string, conversationId: string, messageId: string) {
      applyStorePatch(this, (state) => {
        const conversations = state.conversationsByProjectId[projectId] ?? EMPTY_CONVERSATIONS;
        return {
          conversationsByProjectId: {
