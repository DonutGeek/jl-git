...
describe("isResumeSkillTurn", () => {
  it("显式 @简历时启用技能", () => {
    expect(
      isResumeSkillTurn([
        message("u1", "user", "处理这个项目", [
          { type: "plugin", id: "resume", name: "简历" },
        ]),
      ]),
    ).toBe(true);
  });
  ...
  it("普通 Git 问答出现简历二字不会误触发", () => {
    expect(
      isResumeSkillTurn([
        message("u1", "user", "这个仓库里有个 resume 模板文件吗？"),
      ]),
    ).toBe(false);
  });
...
describe("isSkillCreatorTurn", () => {
  ...
  it("明确创建 Skill 的自然语言请求会启用技能", () => {
    expect(
      isSkillCreatorTurn([
        message("u1", "user", "帮我创建一个 Codex Skill 来审查提交信息"),
      ]),
    ).toBe(true);
  });
  ...
  it("询问仓库中的 SKILL.md 不会误触发", () => {
    expect(
      isSkillCreatorTurn([
        message("u1", "user", "这个仓库里有没有 SKILL.md 文件？"),
      ]),
    ).toBe(false);
  });
