...
function mentionGroupLabelKey(option: { kind?: AgentMentionKind; isRemote?: boolean }): string {
  if (option.kind === "plugin") {
...
function buildMentionSuggestions(
  ...
  const matched = options.filter((option) => {
    ...
    return (
      option.display.toLowerCase().includes(needle) || option.id.toLowerCase().includes(needle)
    );
  });
...
function toAgentMention(
  ...
  if (kind === "plugin" || kind === "skill") {
    const id = mentionId.startsWith("plugin:") ? mentionId.slice("plugin:".length) : mentionId;
    return { type: "plugin", id, name };
  }
  if (kind === "project") {
    const id = mentionId.startsWith("project:") ? mentionId.slice("project:".length) : mentionId;
...
export const AgentComposer = forwardRef<HTMLFormElement, AgentComposerProps>(function AgentComposer(
  {
    draftMarkup,
    draftPlainText,
    branchOptions,
  ...
  const hasPluginOrProject = branchOptions.some(
    (option) => option.kind === "plugin" || option.kind === "skill" || option.kind === "project",
  );
  const emptyMentionsKey = hasPluginOrProject ? "agent.noMentions" : "agent.noBranchMentions";
  const suggestionsA11yKey = hasPluginOrProject ? "agent.mentionsAria" : "agent.branchMentions";
  ...
  function handleInputKeyDown(event: KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>): void {
  ...
  function handleInputWheel(event: WheelEvent<HTMLInputElement | HTMLTextAreaElement>): void {
  ...
  return (
    ...
    >
      {topAccessory ? (
        <div className="mb-2 flex min-w-0 flex-wrap items-center gap-1.5">{topAccessory}</div>
      ) : null}
            ...
            >
              <Mention<AgentMentionOption>
                ...
                renderSuggestion={(option, _query, _highlighted, _index, focused) => {
                  ...
                  const kind =
                    typeof option.kind === "string" ? (option.kind as AgentMentionKind) : undefined;
                    ...
                    <div className="min-w-0">
                      ...
                      <div
                        className={cn(
                          ...
                          focused ? "bg-accent text-accent-foreground" : "hover:bg-accent/60",
                        )}
                      >
        ...
        <div className="flex shrink-0 items-center justify-between gap-2 px-2 pt-1 pb-2">
          ...
          {isReplying ? (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="inline-flex">
                  <Button
                    ...
                    disabled={!effectiveCanSubmit || draftPlainText.trim().length === 0}
                  >
              ...
              <TooltipContent>
                {inputLocked ? t("common.aiApiKeyRequired") : t("agent.sendMessage")}
              </TooltipContent>
            </Tooltip>
          )}
        </div>
