...
/** 只读分支比较的三段式工作区。 */
export function BranchCompareWorkspace({
  project,
  branches,
  initialMode,
  initialBase,
  initialTarget,
}: BranchCompareWorkspaceProps) {
  ...
  const currentBranch =
    branches.find((branch) => branch.isCurrent)?.name ?? localBranches[0]?.name ?? "";
  ...
  const [commitLists, setCommitLists] = useState<{
      ...
      void getBranchCompare(project.path, { base, target: effectiveTarget })
        ...
        .catch((reason: unknown) => {
          if (currentRequest === requestId.current)
            setFileError(toUserMessage(reason) || t("branchCompare.loadFilesFailed"));
        });
      return;
    }
    void Promise.all([
      ...
      .then(([baseOnly, targetOnly]) => {
        if (currentRequest === requestId.current)
          setCommitLists({ baseOnly: baseOnly.commits, targetOnly: targetOnly.commits });
      })
      .catch((reason: unknown) => {
        if (currentRequest === requestId.current)
          setFileError(toUserMessage(reason) || t("branchCompare.loadCommitsFailed"));
      });
  ...
  useEffect(() => {
    ...
    void getBranchFileDiff(project.path, {
      base,
      ...
      .then((result) => {
        if (currentRequest === diffRequestId.current) setDiff(result);
      })
      .catch((reason: unknown) => {
        if (currentRequest === diffRequestId.current)
          setDiffError(toUserMessage(reason) || t("branchCompare.loadDiffFailed"));
      });
  ...
  function selectLocalBranch(nextBase: string): void {
    setBase(nextBase);
    if (mode === "localUpstream")
      setTarget(localBranches.find((branch) => branch.name === nextBase)?.upstream ?? "");
  }

  function swapRefs(): void {
    setBase(effectiveTarget);
    setTarget(base);
  }
  ...
  async function selectCommit(commit: GitCommitSummary): Promise<void> {
    setSelectedCommit(null);
    try {
      setSelectedCommit((await getCommit(project.path, commit.id)).commit);
    } catch (reason) {
      setFileError(toUserMessage(reason) || t("branchCompare.loadCommitFailed"));
    }
  }

  return (
    <main className="bg-background text-foreground flex h-screen min-h-0 w-full min-w-0 max-w-full flex-col overflow-hidden">
      <AppWindowHeader>
        <span
      ...
      <section
        className="border-border grid h-14 shrink-0 grid-cols-[auto_11rem_auto_minmax(0,1fr)_auto_auto_minmax(0,1fr)_auto] items-center gap-2 border-b px-4"
        style={noDragStyle}
      >
        <GitCompareArrows className="size-5 shrink-0" aria-hidden="true" />
        <SelectMenu
          value={mode}
          ariaLabel={t("branchCompare.title")}
          onChange={(value) => setMode(value as BranchCompareMode)}
          options={[
        ...
        <span className="shrink-0 whitespace-nowrap text-sm font-medium">
          {t("branchCompare.source")}
        </span>
        <SelectMenu
          value={base}
          ariaLabel={t("branchCompare.source")}
          onChange={mode === "localUpstream" ? selectLocalBranch : setBase}
          options={
            mode === "localUpstream"
              ? localBranches.map((branch) => ({ value: branch.name, label: branch.name }))
              : allOptions
          }
        ...
        <span className="shrink-0 whitespace-nowrap text-sm font-medium">
          {t("branchCompare.target")}
        </span>
        <SelectMenu
          value={effectiveTarget}
          ariaLabel={t("branchCompare.target")}
          onChange={setTarget}
          disabled={mode === "localUpstream"}
          options={allOptions}
          triggerClassName="min-w-0 w-full"
        ...
        <div className="flex shrink-0 items-center rounded-md border p-0.5 text-sm" role="tablist">
          {(["files", "commits"] as const).map((item) => (
            ...
            >
              {t(`branchCompare.${item}`)}
            </button>
          ))}
        </div>
      </section>
      {mode === "localUpstream" && !effectiveTarget ? (
        <p className="border-border text-muted-foreground shrink-0 border-b px-4 py-2 text-xs">
      ...
      {view === "files" ? (
              ...
              <ScrollArea className="min-h-0 flex-1">
                {fileError ? (
                  ...
                  <EmptyState
                    compact
                    icon={<Files />}
                    title={t("branchCompare.noFiles")}
                    description={t("branchCompare.noFilesDescription")}
                  />
                )}
              </ScrollArea>
            </aside>
            ...
            <section className="min-w-0 h-full">
              {diffError ? (
                ...
                <EmptyState
                  className="h-full"
                  icon={<FileSearch />}
                  title={t("branchCompare.selectFile")}
                  description={t("branchCompare.selectFileDescription")}
                />
              ) : !diff ? (
                <p className="text-muted-foreground p-4 text-sm">{t("branchCompare.loading")}</p>
              ) : (
        ...
        <div className="min-h-0 flex-1 grid grid-cols-[minmax(15rem,1fr)_minmax(15rem,1fr)_minmax(22rem,1.3fr)]">
          <CommitColumn
            title={t("branchCompare.baseOnly", { branch: base })}
            commits={commitLists?.baseOnly ?? []}
            onSelect={selectCommit}
          />
          <CommitColumn
            title={t("branchCompare.targetOnly", { branch: effectiveTarget })}
            commits={commitLists?.targetOnly ?? []}
            onSelect={selectCommit}
          />
...
function CommitColumn({
  ...
  return (
    <section className="border-border min-w-0 border-r">
      <h2 className="border-border border-b px-3 py-2 text-sm font-medium truncate">{title}</h2>
        ...
        {commits.length ? (
          commits.map((commit) => (
            <button
              ...
              className="hover:bg-accent block w-full border-b px-3 py-2 text-left text-xs"
            >
              <p className="text-muted-foreground font-mono">{commit.shortId}</p>
          ...
          <EmptyState
            ...
            title={t("branchCompare.noUniqueCommits")}
          />
        )}
      </ScrollArea>
    </section>
...
function CommitDetail({ commit }: { commit: GitCommitDetail | null }) {
  const { t } = useTranslation();
  return (
    <section className="min-w-0 h-full p-4">
      {!commit ? (
        <EmptyState
          className="h-full"
          icon={<GitCommitHorizontal />}
          title={t("branchCompare.selectCommit")}
          description={t("branchCompare.selectCommitDescription")}
        />
      ) : (
        <>
          <h2 className="text-sm font-semibold">{commit.subject}</h2>
          <p className="text-muted-foreground mt-1 font-mono text-xs">{commit.id}</p>
          <p className="text-muted-foreground mt-3 text-xs">
            {commit.authorName} · {commit.authoredAt}
          </p>
          <pre className="mt-4 whitespace-pre-wrap text-xs">{commit.body}</pre>
        </>
      )}
    </section>
  );
}
function summarizeFiles(files: readonly GitChangedFile[]) {
  return {
    total: files.length,
