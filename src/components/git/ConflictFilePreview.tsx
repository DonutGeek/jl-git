...
type MonacoDecoration = Parameters<Parameters<OnMount>[0]["deltaDecorations"]>[1][number];
...
import { TextDiffPreview, type TextDiffPreviewHandle } from "@/components/git/TextDiffPreview";
import { Spinner } from "@/components/ui/spinner";
import { applyAppMonacoTheme, getAppMonacoThemeName } from "@/design/editor-themes";
import { forceMonacoThemeRepaint } from "@/design/monaco.theme";
import { gitService } from "@/services/git";
import { getActiveThemeChrome, useAppPrefsStore } from "@/store/useAppPrefsStore";
import { useRepoStore } from "@/store/useRepoStore";
import { toUserMessage } from "@/types/error";
import type { GitBlameLine, GitDiffResult } from "@/types/git";
import { patchDiffViewPrefs, readDiffViewPrefs, type DiffViewPrefs } from "@/utils/diffViewPrefs";
import {
...
export const ConflictFilePreview = forwardRef<ConflictFilePreviewHandle, ConflictFilePreviewProps>(
  function ConflictFilePreview(
    { filePath, encoding = DEFAULT_TEXT_ENCODING, onEncodingChange, onBusyChange },
    ref,
  ) {
    ...
    const labelWidgetsRef = useRef<Parameters<Parameters<OnMount>[0]["addContentWidget"]>[0][]>([]);
    const layoutDispRef = useRef<{ dispose: () => void } | null>(null);
    const hunkActionRef = useRef<(index: number, action: ConflictHunkAction) => void>(
      () => undefined,
    );
      ...
      const frame = window.requestAnimationFrame(() => {
        window.requestAnimationFrame(() => {
          ...
          if (monacoRef.current) {
            ...
            applyAppMonacoTheme(monacoRef.current, prefs.appThemeId, getActiveThemeChrome(prefs));
          }
    ...
    async function handleHunkAction(index: number, action: ConflictHunkAction): Promise<void> {
      if (!repoPath || busy || text == null) {
        return;
      }
      setBusy(true);
        ...
        options: {
          ...
          hoverMessage: {
            value: t("repo.diffBlameHover", {
              ...
              hash: line.shortId,
              time: line.authoredAt ? dayjs(line.authoredAt).format("YYYY-MM-DD HH:mm") : "—",
            }),
          },
        },
      }));
    ...
    return (
      <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
        <DiffPreviewToolbar
          ...
          diffToolsDisabled={mode !== "diff" || diffLoading || Boolean(diff?.binary) || !diff}
