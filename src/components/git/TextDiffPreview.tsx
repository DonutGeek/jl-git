...
export const TextDiffPreview = forwardRef<TextDiffPreviewHandle, TextDiffPreviewProps>(
  function TextDiffPreview(
    ...
    const [innerDiffLayout, setInnerDiffLayout] = useState<DiffPreviewLayout>("sideBySide");
    ...
    const handleDiffMount: DiffOnMount = (editor, monaco) => {
      ...
      const syncChanges = (): void => {
        ...
        if (lineChanges.length > 0 && revealedSelectionRef.current !== selectionKey) {
        ...
        const lineHeight = Math.max(1, modified.getOption(monaco.editor.EditorOption.lineHeight));
        ...
        for (const change of lineChanges) {
          ...
          if (hasOriginal) {
            ...
            markers.push({
              ...
              heightRatio: Math.max(2 / scrollHeight, (originalCount * lineHeight) / scrollHeight),
              kind: "delete",
            });
          }
    ...
    function jumpModified(ratio: number): void {
      ...
      const maxScroll = Math.max(0, modified.getScrollHeight() - modified.getLayoutInfo().height);
    ...
    useEffect(() => {
      ...
      if (!viewPrefs.lineBlame || blameLines.length === 0) {
        blameDecorationsRef.current = target.deltaDecorations(blameDecorationsRef.current, []);
        return;
      }
