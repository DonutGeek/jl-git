...
export function TagList({ onSelectTag }: TagListProps) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <ScrollArea className="min-h-0 flex-1 px-1 py-0.5">{tags.length === 0 ? <p className="text-muted-foreground px-2 py-3 text-xs">{t("repo.tagsEmpty")}</p> : filtered.length === 0 ? <p className="text-muted-foreground px-2 py-3 text-xs">{t("repo.tagsNoMatch")}</p> : <div className="flex flex-col gap-0.5">{filtered.map((tag) => <div key={tag.name} className={cn("group flex items-center gap-2 rounded-md px-2 py-1.5", logRef === tag.name ? "bg-primary/15" : "hover:bg-accent/60")}><button type="button" className="flex min-w-0 flex-1 items-center gap-2 text-left text-sm" onClick={() => void select(tag.name)}><Tag className="text-muted-foreground size-4 shrink-0" /><span className="truncate">{tag.name}</span></button><Tooltip><TooltipTrigger asChild><Button type="button" variant="ghost" size="icon" className="size-6 opacity-0 group-hover:opacity-100" aria-label={t("repo.deleteTag")} disabled={busyName === tag.name} onClick={() => void remove(tag.name)}><Trash2 className="size-3.5" /></Button></TooltipTrigger><TooltipContent>{t("repo.deleteTag")}</TooltipContent></Tooltip></div>)}</div>}</ScrollArea>
