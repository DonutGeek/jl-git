...
- 数据库位于应用数据目录；备份包含数据库、Store 密钥与偏好，ZIP 本身不加密，必须提示用户妥善保管
- 导出先写唯一临时文件并落盘，完整成功后再可回滚地替换目标，失败不得破坏已有备份
- 导入仅接受清单约定的固定文件名，拒绝越界/重复条目并限制单项及总解压体积
- 导入数据库先校验格式并写入 pending；启动连接/迁移失败时自动回滚导入前数据库
