...
### `getLog(repoPath, options?: GitLogOptions): Promise<{ commits; hasMore }>`
...
- **说明：** Store / 历史列表用的查询构造器；`logRef == null` → `{ all: true }`（所有分支，冷启动默认），否则 → `{ ref: logRef }`。附带 `order`（`default` 时不传给后端）。
