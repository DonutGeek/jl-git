...
**Architecture:** Settings → `openResumeHelperWindow` → `/resume-helper` → Workspace（复用 Agent 消息 UI）→ `buildResumeProfiles`（前端调现有 git_log/listTree）→ `streamResumeHelperReply`（独立 prompt + 鲸灵 Key）。
