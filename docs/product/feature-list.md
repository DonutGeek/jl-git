...
| 工作区切换 | Done | 工具栏三视图：工作区浏览器（含文件预览）/ 变更 / 历史 |
| 空状态引导 | Done | |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 导入已有 Git 仓库 | Done | 新标签页「打开」登记本地路径 |
| 克隆远程仓库 | Done | 新标签页「克隆」：地址 + 存放路径；可选「克隆后填写项目详情」弹窗（AI 生成 / 手填 / 跳过）；「克隆并打开」/「克隆并继续」 |
| 重命名显示名 | Done | 仓库设置 / 仓库管理页可改别名（不改文件夹名） |
| 改绑本地路径 | Done | 编辑抽屉可改/选择路径；须为 Git 根；比对主远端，不一致需确认 |
| 从应用移除 | Done | 右键或仓库管理页删除记录（不删磁盘） |
| 仓库管理页 | Done | 浮动子窗项目管理控制台：筛选（关键词/分组/Git 状态）与实况表格；打开/克隆/导入导出登记清单/刷新；打开仓库回到主窗 |
| 仓库登记清单导入导出 | Done | JSON（`jlgit.project-catalog`）：导出全量或当前筛选；导入预览勾选；路径无效不可导入；分组名称优先 + id 兜底 |
| 打开失败修复 | Planned | 路径失效 |
...
| 主区三视图骨架 | Done | 工作区目录浏览 + 文件预览；变更 Diff；历史详情 |
| 工作区文件预览 | Done | 目录树/网格点击文件；复用对比工具（编码 / 历史） |
| 外部打开仓库 | Done | 访达 / 资源管理器 / 文件管理器；编辑器与终端读取设置偏好（按平台选项） |
| 侧栏活动栏（目录树 / 分支 / Agent） | Done | 最左图标栏切换；Agent 对话列表使用虚拟滚动 |
| 目录树 | Done | 懒加载；点击文件打开工作区预览；右键复制路径 / 访达 / 外部编辑器；文件夹可新建空目录/文件、重命名、删除 |
...
| 编写并提交 message | Done | |
| Amend（可选） | Partial | 历史右键可改 HEAD 提交信息；非 HEAD / 交互变基 reword 未做 |
| 丢弃更改 | Planned | 二次确认 |
...
| 鲸灵（单仓/多仓） | Done | 同一套 Agent；默认只读 Git + 读代码理解（无插件）；「简历生成」与「技能创建」使用互相隔离的独立 Prompt；简历生成默认用仓库/全局 Git 身份匹配个人提交（可对话覆盖）；技能创建生成可落盘 Skill 包内容但不直接写盘；会话按宿主分桶 |
