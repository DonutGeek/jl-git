...
- **插件壳：** 「新增会话」下方「插件」进入目录；内含「插件 / 技能」分段切换（共用 `AgentCatalogPanel`）。内置技能「简历」「技能创建」；插件目录可为空态。单仓可用 `@简历` / `@技能创建` / `@分支`；多仓可再 `@项目名`。明确的简历成稿或创建/更新 Skill 请求也可自然语言触发；无 Composer 快捷 chip。不做插件市场。
...
- **对话范围与隔离：** 默认围绕 Git / 仓库正常交流（状态、分支、提交、diff、冲突等）；纯问候可简短回应并引导到 Git 问题；非 Git 话题礼貌收回。只有用户 `@` 技能或明确提出对应任务时，本轮才切换能力。通用单仓/多仓 Prompt 不包含简历规则、作者匹配、`matchedCommits`、个人贡献判断，也不包含 Skill 文件结构、创建工作流或输出契约；未启用技能时就是专业 Git Agent。简历与技能创建也不共享各自的身份、工作流或会话续接标记。
...
- **简历身份（硬边界）：** 简历技能**不读取**当前仓库 `user.name/user.email`、全局 `git config` 或「设置 → Git」账号。用户必须在对话中主动声明自己的 Git 作者名或提交邮箱（可多个）；身份缺失时只追问一次必要信息，不扫描个人提交、不猜测、不成稿。身份声明后的紧邻回复可延续原简历请求，无需再次 `@简历`。
...
- **单仓**每次请求先注入当前项目的 **`jlgitMeta`（路径 / 别名 / 分组 / 项目详情）**，再读取只读 Git 快照（状态、分支、近期提交；询问文件列表或启用技能创建时拉 HEAD 文件树），不得混入其它项目。识别项目名与简介时优先登记详情与别名，勿用 README 覆盖用户手填简介。用户询问最近一次提交或引用近期短 SHA 时按需补文件列表；追问具体改动时最多 6 个文件限长 patch。工作区/未提交变更同理。**通用模式**另提供只读代码工具 `list_dir` / `read_file` / `search_code`（DeepSeek function calling；工具轮非流式，结果脱敏后回灌），用于定位页面与实现逻辑；禁止写盘、越界、读取 `.env`/密钥/二进制。回复用通俗语言直接答问；发送时先上屏用户消息再异步拉快照。简历技能启用且身份已声明时，才用 `git log --all --author` 读取该作者提交与参与周期。
- **多仓**默认是**多仓 Git Agent**（解释项目、跨仓对照、只读画像问答）。普通画像始终按仓库整体构建，不受个人身份影响。代码工具须先 `@项目`（或正文点名 / 仅一仓），`codeToolRoots` 仅含目标仓；未锁定目标时不开放工具。仅当用户启用简历技能且主动声明身份后，才按声明的作者过滤器延迟构建、缓存本轮简历画像；每仓仍注入提交抽样、技术栈、README 摘录，以及同样的 **`jlgitMeta`**。识别项目名与简介时优先 `jlgitMeta.description` 与 `alias`，其次 README；禁止跨技能复用个人作者上下文。
