...
| 插件 / 技能 | 同一套壳；简历归技能 | 同左；简历可点名多仓 |
| 会话桶 | `project_id` 必填；删项目 CASCADE | `project_id` 空；仅手动删 |

- **插件壳：** 「新增会话」下方「插件」进入目录；内含「插件 / 技能」分段切换（共用 `AgentCatalogPanel`）。首期内置技能「简历」；插件目录可为空态。单仓可用 `@简历` / `@分支`；多仓可用 `@简历` / `@项目名` 或自然语言触发；无 Composer 快捷 chip。不做插件市场。
...
- **对话范围：** 默认围绕 Git / 仓库正常交流（状态、分支、提交、diff、冲突等）；纯问候可简短回应并引导到 Git 问题；非 Git 话题礼貌收回。用户 `@` 插件/技能（或明确调用）时，本轮切换到对应能力（如 `@简历` → 简历技能 prompt）；未启用时不推销简历。普通多仓对话**不**注入/讨论 Git 作者归属与 `matchedCommits`（「谁的提交」仅简历技能需要）。
- **单仓**每次请求读取**当前项目**只读 Git 快照（状态、分支、近期提交；询问文件列表时才拉 HEAD 文件树），不得混入其它项目。用户询问最近一次提交或引用近期短 SHA 时按需补文件列表；追问具体改动时最多 6 个文件限长 patch。工作区/未提交变更同理。回复用通俗语言直接答问；发送时先上屏用户消息再异步拉快照。单仓同样支持 `@简历` / 明确成稿意图切换简历技能。
- **多仓**默认是**多仓 Git Agent**（解释项目、跨仓对照、只读画像问答），**不得**在未启用简历技能时推销「生成简历」。仅当用户 `@简历` 或明确要求生成简历/项目经历时，才切换到简历技能的独立 system prompt 与成稿规则。每次请求注入已登记仓库只读画像：提交抽样、技术栈、README 摘录，以及 **`jlgitMeta`（鲸灵Git 登记信息：绝对路径 / 别名 / 分组；可扩展）**。识别项目名与简介时优先 `jlgitMeta`，其次 README。
