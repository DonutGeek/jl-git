...
- **持久化：** 鲸灵与鲸履会话写入 SQLite（`chat_conversations` / `chat_messages`，含 `reasoning_content`）。鲸灵绑定 `project_id`，删除 Git 项目时 CASCADE 清理；鲸履 `project_id` 为空，仅用户手动删会话。打开项目 / 打开鲸履子窗时 hydrate 到 Zustand；消息完成、停止保留片段、编辑截断、重命名/置顶/重排/删除时 upsert，不写流式中间帧。
- 每次 DeepSeek 请求还会读取**当前项目**的只读 Git 快照（状态、分支、近期提交；询问文件列表时才拉 HEAD 文件树），不会混入其它项目的会话或仓库数据。用户询问最近一次提交，或在当前会话中引用近期提交的短 SHA 时，会按需补充该提交的真实文件列表；询问具体改动时，最多补充 6 个文件的限长 patch。用户询问**当前工作区 / 未提交变更**时，状态条目以可读标签提供；仅在追问具体/行级差异时再补充最多 6 个文件的限长 patch。回复应直接用通俗语言回答用户所问：不交代信息来源（除非用户追问）、不答非所问、不额外发散；发送时先同步上屏用户消息，再异步拉快照与请求模型，保证回车响应灵敏。
- 用户提问中明确出现两个已知分支名时，鲸灵会额外读取两个方向的提交区间，用于说明双方各自独有的提交；这仅用于分析，不会执行任何 Git 写操作。
- 需要 Git 问答时，必须显式传入当前项目的仓库路径与用户选择的上下文，禁止根据其它打开标签或全局缓存推断上下文。
...
- 模型：提交文案为 `deepseek-chat`；**鲸灵 Agent** 与 **鲸履** 为 `deepseek-v4-pro`（共用 `AgentReasoningBlock` 展示可折叠深度思考，正文流式输出）。鲸灵输入框左下角可切换「深度思考」（默认开；关闭时同模型 `thinking: disabled`）。助手末条可「重新生成」；用户消息可内联「修改」后截断后续并重发（无版本切换）
- 设置项：用户可创建多个 `DeepSeek API Key`（名称、Key、创建日期）；同一时刻仅允许一个 Key 启用，启用新 Key 会自动禁用其它 Key；删除需二次确认
- 余额：设置 → 鲸灵 用启用 Key 调用 `GET /user/balance` 展示（优先 CNY）；「去充值」打开 [DeepSeek 充值页](https://platform.deepseek.com/top_up)
