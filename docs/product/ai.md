...
- **简历身份：** 默认读取当前仓库生效的 `user.name` / `user.email`（单仓）或全局身份（多仓；全局为空时回退已登记仓本地配置）。用户也可在对话中主动声明作者名/邮箱以覆盖。仅当两者皆无时追问一次；不猜测、不编造身份。身份确认后的紧邻回复可延续原简历请求，无需再次 `@简历`。
...
- **单仓**每次请求先注入当前项目的 **`jlgitMeta`（路径 / 别名 / 分组 / 项目详情）**，再读取只读 Git 快照（状态、分支、近期提交；询问文件列表或启用技能创建时拉 HEAD 文件树），不得混入其它项目。识别项目名与简介时优先登记详情与别名，勿用 README 覆盖用户手填简介。用户询问最近一次提交或引用近期短 SHA 时按需补文件列表；追问具体改动时最多 6 个文件限长 patch。工作区/未提交变更同理。**通用模式**另提供只读代码工具 `list_dir` / `read_file` / `search_code`（DeepSeek function calling；有 tool_calls 时不向 UI 推半截正文，最终成稿 SSE 流式上屏；结果脱敏后回灌），用于定位页面与实现逻辑；禁止写盘、越界、读取 `.env`/密钥/二进制。回复用通俗语言直接答问；发送时先上屏用户消息再异步拉快照。简历技能启用且已解析作者身份时，才用 `git log --all --author` 读取该作者提交与参与周期。
- **多仓**默认是**多仓 Git Agent**（解释项目、跨仓对照、只读画像问答）。普通画像始终按仓库整体构建，不受个人身份影响。代码工具须先 `@项目`（或正文点名 / 仅一仓），`codeToolRoots` 仅含目标仓；未锁定目标时不开放工具。仅当用户启用简历技能且已解析作者身份后，才按作者过滤器延迟构建、缓存本轮简历画像；每仓仍注入提交抽样、技术栈、README 摘录，以及同样的 **`jlgitMeta`**。识别项目名与简介时优先 `jlgitMeta.description` 与 `alias`，其次 README；禁止跨技能复用个人作者上下文。
